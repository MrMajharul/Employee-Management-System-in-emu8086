\begin{titlepage}
\center 
\newcommand{\HRule}{\rule{\linewidth}{0.1mm}}
\includegraphics[scale=0.2]{Figures/GUBLogo.png}\\[1cm] 
\center 
%\quad\\[1.5cm]
\textsl{\Large Green University of Bangladesh }\\[0.5cm] 
\textsl{\large Department of Computer Science and Engineering (CSE)}\\
%\textsl{\large Faculty of Sciences and Engineering (FSE)}\\
\textsl{\large Semester: (Fall, Year: 2025), B.Sc. in CSE (Day)}\\[0.5cm] 
\makeatletter
\HRule \\[0.4cm]
{ \huge \bfseries Employee Management System}\\[0.2cm] 
\HRule \\[1.0cm]

\textsl{\large Course Title: Microprocessors \& Microcontrollers Lab }\\
\textsl{\large Course Code: CSE 304 }\\ 
\textsl{\large Section: 232 D4 }\\[0.5cm] 

{\large \underline{Students Details}}\\[0.2cm]

\begin{table*}[htb]
\centering
\begin{tabular}{ |P{7.0cm}|P{3.5cm}|}
\hline
\textbf{Name} & \textbf{ID}\\
\hline
Bakul Ahmed  & 232002184 \\
Majharul Islam & 232002256\\
\hline
\end{tabular}
\end{table*}
\vspace{0.5cm}

\textsl{\large Submission Date: 03-12-25 }\\ 
\textsl{\large Course Teacher’s Name: Jarin Tasnim Tanvi }\\[0.9cm] 

\vspace{4cm}

\makeatother
{\large [For teachers use only: \textcolor{red}{Don’t write anything inside this box]}}\\

\begin{table}[]
\centering
\begin{tabular}{|p{7.5cm}p{7.0cm}|}
\hline
\multicolumn{2}{|c|}{{\underline{\textbf{Lab Project Status}}}} \\
 & \\\hline
\textbf{Marks:}                & \textbf{Signature:}        \\
 & \\ 
\textbf{Comments:}             & \textbf{Date:}             \\ 
 & \\\hline
\end{tabular}
\end{table}


\end{titlepage}

